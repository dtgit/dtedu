...
try:
    ...
    context.acl_users.userFolderEditUser(username, password, ['Authenticated', 'Member', 'Contributor'], [])
...
if portal.validate_email or REQUEST.get('mail_me', 0):
    try:
        ...
        if portal.validate_email:
            ...
            msg = _(u'status_fatal_password_mail',default=u'Failed to create your account: we were unable to send your password to your email address: ${address}',
                    mapping={u'address' : str(err)})
            context.plone_utils.addPortalMessage(msg, 'error')
            return state.set(status='failure')
        else:
            msg = _(u'status_nonfatal_password_mail',u'Your account has been created, but we were unable to send your password to your email address: ${address}',
                    mapping={u'address' : str(err)})
            context.plone_utils.addPortalMessage(msg, 'error')
