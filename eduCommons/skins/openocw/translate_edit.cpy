...
sel = new_context.getCanonical()

translation = sel.getTranslation(lp_translating_from)
if sel.hasTranslation(lp_translating_from):
    if translation != sel:
        raise 'Already Translated'
else:
    sel.setLanguage(lp_translating_from)
