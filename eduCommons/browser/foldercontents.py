...
class ISyncOrderView(Interface):
    """ A interface class for the sync form """


    
class SyncOrderView(AddForm):
    """ A form for synching up order when there is a discrepancy. """

    form_fields = FormFields(ISyncOrderView)
    label = _(u'Course Ordering Information Missing')
    description = _(u'There are items in your course that have no ordering information, click submit to fix this problem. The items will need to be reordered.')

    @action(_(u'Submit'), 
            name=u'Submit')
    def action_submit(self, action, data):

        brains = self.context.portal_catalog.searchResults({'path':{'query':'/'.join(self.context.getPhysicalPath())+'/'},'excludeFromNav':'False'})
        pos = 1

        for brain in brains:
            obj = brain.getObject()
            if obj.portal_type != 'Course' and not obj.getExcludeFromNav():
                IAnnotations(obj)['eduCommons.objPositionInCourse'] =  pos
                obj.reindexObject()
                pos += 1
            elif obj.portal_type == 'Course':
                IAnnotations(obj)['eduCommons.objPositionInCourse'] = 0
                obj.reindexObject()

        IStatusMessage(self.request).addStatusMessage(_(u'Course item reorder has completed successfully'), type='info')
