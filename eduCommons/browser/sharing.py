...
class SharingView(BrowserView):
        ...
        if submitted and not cancel_button:
            ...
            for entry in entries:
                settings.append(
                    dict(id = entry['id'],
                         type = entry['type'],
                         roles = [r for r in roles if entry.get('role_%s' % 'Share', False)]))
                         #Simplifying to one flag : Can Share'
    ...
    def user_search_results(self):
        ...
        #Directly call acl_users since SQLPas doesn't provide search
        for userinfo in acl_users.searchUsers(id=search_term):
