...
class SearchView(BrowserView):
    ...
    def isMemberLocallyAllowed(self):
        """
        Tests to see if the object is addable to member folders
        """
        if self.context.portal_membership.isAnonymousUser():
            return False

	props = self.context.portal_properties
        if self.context.Type() in props.educommons_properties.member_locally_allowed_types:
            return True
        else:
            return False
    ...
    def changeNav(self, nav_action):
        if nav_action == 'hide':
            ...
            IAnnotations(self.context)['eduCommons.objPositionInCourse'] = -1
        else:
            ...
            if self.context.getExcludeFromNav():
                ...
                IAnnotations(self.context)['eduCommons.objPositionInCourse'] = self.appendObjPosition(self.context)
            else:
                if not IAnnotations(self.context).has_key('eduCommons.objPositionInCourse'):
                    IAnnotations(self.context)['eduCommons.objPositionInCourse'] = self.appendObjPosition(self.context)
                message = 'Object(s) already set to display in left hand naviagtion'
