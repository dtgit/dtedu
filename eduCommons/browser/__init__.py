...
class PositionView(BrowserView):
    ...
    def moveUp(self, context, position, id, path):
        ...
        for obj in objs[1:]:
            if path == obj.getPath():
                if not self.swapPosition(obj.getObject(), prev_obj.getObject()):
                    context.request.response.redirect('order_sync_view')
                    return
                break
    ...
    def moveDown(self, context, position, id,path):
        ...
        for obj in objs:
            ...
            if prev_obj:
                if not self.swapPosition(obj.getObject(), prev_obj.getObject() ):
                    self.context.request.response.redirect('order_sync_view')
                    return
                break
            if path == obj.getPath():
                prev_obj = obj
    ...
    def swapPosition(self, cur_obj, swp_obj):
        try:
            ...
            swp_pos = IAnnotations(swp_obj)['eduCommons.objPositionInCourse']        
        except KeyError:
            return False
        ...
        IAnnotations(cur_obj)['eduCommons.objPositionInCourse'] = swp_pos
